"""Backtest with CORRECTED reverse zodiac mapping"""
...
for r in raw_data:
    exp = r.get("expect", "")
    if exp not in seen: seen.add(exp); data.append(r)
...
class CorrectedPredictor:
    ...
    def predict_specials(self, count=8):
        ...
        for r in self.a.data[-20:]:
            for n in self.a.get_numbers(r): all_freq[n] += 1
...
# RUN
print("="*60)
print("CORRECTED ZODIAC (REVERSE CYCLE) BACKTEST")
print("="*60)
warmup = 60
...
for idx in range(warmup, len(data)):
    ...
    try:
        if actual[6] in CorrectedPredictor(a).predict_specials(8):
            h += 1
        t += 1
    except: pass
print(f"CorrectedPredictor: {h}/{t} = {h/t*100:.1f}%")

# Zodiac validation
print("\n=== Zodiac Mapping Validation ===")
matches = 0; total = 0
for r in data:
    dt = r.get("openTime",""); code = r.get("openCode",""); dz = r.get("zodiac","")
    if code and dz:
        nums = [int(x) for x in code.split(",") if x.strip()]
        data_zods = [z.strip() for z in dz.split(",")]
        for n, dz2 in zip(nums, data_zods):
            cz = num_to_zodiac(n, dt)
            total += 1
            if cz == dz2: matches += 1
print(f"Zodiac match rate: {matches}/{total} = {matches/total*100:.1f}%")
...
for idx in range(w_start, len(data)):
    ...
    if len(actual) < 7: continue
    try:
        if actual[6] in CorrectedPredictor(a).predict_specials(8): h50 += 1
        t50 += 1
    except: pass
print(f"Recent 50: {h50}/{t50} = {h50/t50*100:.1f}%")
